...
def generate_impostor_message(context_messages: list[dict]) -> str:
    ...
    if memory_context:
        prompt_context += f"What {impostor.disguised_as} might remember:\n{memory_context}\n\n"
    
    prompt = f"{prompt_context}Now naturally join or comment on this conversation as {impostor.disguised_as}."
...
def activate_impostor(
    target_player_id: Optional[str] = Body(None, embed=True),
    engagement_rate: float = Body(0.3, embed=True)
):
    """
...
def update_impostor_settings(
    message_cooldown: Optional[float] = Body(None, embed=True),
    engagement_rate: Optional[float] = Body(None, embed=True)
):
